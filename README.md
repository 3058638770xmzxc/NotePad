...
- 📝 **笔记管理**：创建、编辑、删除笔�?- 🔍 **搜索功能**：支持全局搜索和标签内搜索
- 🏷�?**标签分类**：支持标签创建和笔记分类
- 🎨 **个性化定制**：可自定义背景颜色和背景图片
- ⚙️ **排序设置**：支持最新优先和最旧优先排�?- 📱 **手势操作**：左右滑动隐�?显示列表
- 💾 **数据安全**：自动保存和未保存修改提�?- 🖋�?**编辑器设�?*：支持编辑页背景颜色和字体大小调�?
## 技术栈

- Java
- Android SDK
- SQLite数据�?- ContentProvider
- CursorAdapter
...
**界面说明**�?- 顶部自定义ActionBar，显示应用名�?- 笔记列表以卡片形式展示，包含标题、修改时间和标签
- 右上角有添加新笔记的"+"图标
- 支持左右滑动手势隐藏/显示列表
...
**操作流程**�?
1. **进入新建笔记界面**
   - 截图：screenshots/)P1E_KJK0CC`W8VC@FOMOE0.png
   - 操作：点击主界面右上角的"+"图标
   - 效果：进入空白的笔记编辑界面
   - **关键代码**�?     ```java
...
3. **添加标签**
   - 截图：screenshots/P3AT~{G9O2$D5MKR]75B6LP.png
   - 操作：点击标签按钮，选择已有标签或创建新标签
   - 效果：标签添加到笔记�?   - **关键代码**�?     ```java
     // 在NoteEditor.java中处理标签设�?     mTagButton = (Button) findViewById(R.id.tag_button);
     if (mTagButton != null) {
...
4. **修改笔记标题**
   ...
   - 操作：点击标题区域，输入新标�?   - 效果：标题更�?   - **关键代码**�?     ```java
     // 在NoteEditor.java中处理标题编�?     mActionBarTitle = (TextView) customView.findViewById(R.id.actionbar_title);
...
5. **保存笔记**
   - 截图：screenshots/V2~{{}P`_7LFW$A`97YA[C9.png
   - 操作：点击顶部ActionBar�?保存"按钮
   - 效果：笔记保存到数据库，返回主界�?   - **关键代码**�?     ```java
     // 在NoteEditor.java中保存笔�?     @Override
...
- **向左滑动**：隐藏笔记列表，显示空背�?  - 截图：screenshots/JZ5C6}@4G]WK%B2{_{[)@13.png
  - 操作：在主界面从右向左滑�?  - 效果：笔记列表向左滑出屏幕，只显示背�?  - **关键代码**�?    ```java
...
- **向右滑动**：恢复笔记列�?  - 操作：当列表隐藏时，从左向右滑动
  - 效果：笔记列表平滑滑入屏幕，恢复正常显示
  - **关键代码**�?    ```java
    // 在NotesList.java中处理右滑恢复列�?    @Override
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        // ... 前面的代�?...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > SWIPE_MIN_DISTANCE) {
            if (deltaX > 0) {
...
- 截图：screenshots/%8E}Y07FFQ$GOSE81_KP{1G.png
- 操作�?  1. 在主界面点击搜索图标
  2. 输入搜索关键�?  3. 系统在所有笔记中搜索
- 效果：显示所有匹配关键词的笔�?- **关键代码**�?  ```java
  // 在NotesList.java中处理搜索意�?  private void handleIntent(Intent intent) {
      if (Intent.ACTION_SEARCH.equals(intent.getAction())) {
          String query = intent.getStringExtra(SearchManager.QUERY);
          if (query != null && !query.isEmpty()) {
              String searchPattern = "%" + query + "%";
              // 全局搜索：在标题、内容和标签中搜�?              mSelection = NotePad.Notes.COLUMN_NAME_TITLE + " LIKE ? OR " + 
...
#### 4.2 标签内搜�?
- 截图：screenshots/6D)_SCZD4DYOK$`%3~@]X]0.png
- 操作�?  1. 点击标签进入标签过滤状�?  2. 点击搜索图标
  3. 输入搜索关键词（�?周二"�?- 效果：只在当前标签内搜索，显示匹配结�?- **关键代码**�?  ```java
...
- 截图：screenshots/KDK)HVZB`YM~9K}PV68~0XV.png
- 操作�?  1. 进入设置界面
  2. 找到"排序方式"选项
  3. 选择"最新优�?�?最旧优�?
- 效果：笔记列表按选择的方式排�?- **关键代码**�?  ```java
  // 在NotesList.java中获取排序方�?  private String getSortOrder() {
      if (prefs == null) {
...
- 操作�?  1. 进入设置界面
  2. 点击"背景颜色"选项
  3. 从颜色选择器中选择颜色（白色、米色、蓝色、绿色等�?  4. 系统自动应用所选颜�?- **关键代码**�?  ```java
  // 在NotesList.java中应用背景颜�?  private void applyBackgroundColor() {
      if (prefs == null) {
          return;
      }
...
- 操作�?  1. 进入设置界面
  ...
  4. 进入图片裁剪界面，调整裁剪框
  5. 点击"保存"完成裁剪
- 效果：应用背景变为所选图�?- **关键代码**�?  ```java
  // 在NotesList.java中应用背景图�?  private void applyBackgroundColor() {
      // ... 背景颜色代码 ...
...
### 6. 笔记编辑页设�?
- 截图：screenshots/F$L[FHZV0}G_MXIP(7V8G(8.png)、screenshots/RT%SON24_O5Y41R3]@7VBLI.png
- 操作�?  1. 进入笔记编辑界面
  2. 点击设置图标
  3. 调整编辑页背景颜�?  4. 调整笔记字体大小
- 效果：实时预览并保存设置
- **关键代码**�?  ```java
...
- 截图：screenshots/FT%FS%0(%JJ)04CSBA{J@WU.png
- 操作�?  1. 在笔记编辑界面点击删除图�?  2. 弹出删除确认对话�?  3. 确认后删除笔�?- 效果：笔记从数据库中删除，返回主界面
- **关键代码**�?  ```java
  // 在NoteEditor.java中处理删除操�?  @Override
  public boolean onOptionsItemSelected(MenuItem item) {
      if (item.getItemId() == R.id.menu_delete) {
          // 显示删除确认对话�?          new AlertDialog.Builder(this)
...
### 8. 未保存修改提�?
- 截图：screenshots/_0Q4QTW3EOH[OF(J%0(JB76.png)、screenshots/]U$)B6DG(]S$ZLGOI{UWHOH.png
- 操作场景：在笔记编辑界面修改内容后，未点击保存而点击返�?- 效果�?  - 弹出提示对话框："您有未保存的更改"
  - 提供"保存"�?不保�?选项
  - 点击"保存"：保存更改并返回
  - 点击"不保�?：放弃更改并返回
- **关键代码**�?  ```java
  // 在NoteEditor.java中处理返回按�?  private void handleBackPressed() {
      // 检查内容是否有变化
...
public class NotePadProvider extends ContentProvider {
    // 数据库版�?    private static final int DATABASE_VERSION = 2;
    // 数据库名�?    private static final String DATABASE_NAME = "note_pad.db";
    // 表名
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        // 构造函�?        DatabaseHelper(Context context) {
...
gestureDetector = new GestureDetector(this, new GestureDetector.SimpleOnGestureListener() {
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        // 检测水平滑�?        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > SWIPE_MIN_DISTANCE) {
...
private SimpleCursorAdapter createNotesAdapter(Cursor cursor) {
    ...
    return new SimpleCursorAdapter(
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            // 显示标签并添加点击事�?            TextView tagView = view.findViewById(R.id.tag_text);
            if (tagView != null) {
...
�?  �?  ├── NotesList.java          # 笔记列表主界�?�?  �?  ├── NoteEditor.java         # 笔记编辑界面
�?  �?  ├── NotePadProvider.java    # 数据库内容提供�?�?  �?  ├── NotePad.java            # 常量定义
�?  �?  ├── TitleEditor.java        # 标题编辑界面
�?  �?  ├── SettingsActivity.java   # 设置界面
�?  �?  ├── EditorSettingsActivity.java # 编辑器设�?�?  �?  └── ImageCropActivity.java  # 图片裁剪功能
�?  └── res/                        # 资源文件
└── build.gradle                    # 构建配置
```
...
1. Android数据库操�?2. ContentProvider的使�?3. ListView和CursorAdapter的应�?4. 自定义视图和动画
5. 搜索功能实现
6. 手势操作处理
7. 自定义主题和样式
